...
import { VerificationBadge, UnverifiedWarning, LocationVerificationInfo } from '@/app/components/VerificationBadge';
import { BookingDialog } from '@/app/components/BookingDialog';
import { GreenBadgeDisplay } from '@/app/components/GreenBadgeDisplay';
...
import { getLandlordById, getPropertiesByLandlord } from '@/app/components/mockData';
...
import {
  ArrowLeft, Heart, Share2, MapPin, Phone, Calendar, User, Star,
  Wifi, Tv, WashingMachine as WashingMachineIcon, Utensils, Refrigerator, Wind, Sofa,
  ChevronLeft, ChevronRight, X, Expand, Clock, MessageCircle, Shield,
  ShieldCheck, Copy, Check, ExternalLink, Home,
  Hospital, GraduationCap, ShoppingCart, TreePine, Bus
} from 'lucide-react';

// --- Mock reviews ---
const mockReviews = [
  { id: 'r1', author: 'Minh Tuấn', rating: 5, date: '2026-02-10', text: 'Phòng sạch sẽ, chủ nhà thân thiện. Giá hợp lý so với khu vực. Rất recommend!', avatar: 'MT' },
  { id: 'r2', author: 'Hương Ly', rating: 4, date: '2026-01-28', text: 'Vị trí thuận tiện, gần chợ và trường học. Phòng hơi nhỏ nhưng đầy đủ tiện nghi.', avatar: 'HL' },
  { id: 'r3', author: 'Thanh Hà', rating: 5, date: '2026-01-15', text: 'Đã ở 6 tháng rồi, rất hài lòng. Chủ nhà sửa chữa nhanh khi có vấn đề. An ninh tốt.', avatar: 'TH' },
  { id: 'r4', author: 'Quốc Bảo', rating: 3, date: '2025-12-20', text: 'Phòng OK, nhưng wifi đôi khi chập chờn. Hy vọng sẽ được cải thiện.', avatar: 'QB' },
];
...
function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star key={i} className={`size-4 ${i <= rating ? 'fill-amber-400 text-amber-400' : 'text-gray-300'}`} />
      ))}
    </div>
  );
}
...
// --- Mini Map ---
function MiniMap({ property }: { property: RentalProperty }) {
  ...
  const nearbyFacilities = useMemo(() => {
    return mockFacilities
      .map(f => ({
        ...f,
        distance: getDistanceKm(property.location[0], property.location[1], f.lat, f.lng),
      }))
  ...
  useEffect(() => {
    ...
    const facilityColors: Record<string, string> = {
      hospital: '#ef4444', school: '#f59e0b', supermarket: '#8b5cf6', park: '#10b981', bus_stop: '#3b82f6',
    };
    ...
    nearbyFacilities.forEach(f => {
      ...
      L.marker([f.lat, f.lng], { icon: fIcon })
        .addTo(mapRef.current!)
        .bindPopup(`<strong>${f.name}</strong><br/>${Math.round(f.distance * 1000)}m`);
    });
          ...
          {nearbyFacilities.map(f => {
            ...
            return (
              <div key={f.id} className="flex items-center gap-3 p-2.5 bg-gray-50 rounded-lg">
                <div className={`flex-shrink-0 ${config?.color || 'text-gray-500'}`}>
...
export function RoomDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'reviews' | 'nearby'>('info');
  const [isRequestInspectionOpen, setIsRequestInspectionOpen] = useState(false);

  // Find property from context instead of mock data
  const property = properties.find(p => p.id === id);
  const landlord = property?.landlordId ? getLandlordById(property.landlordId) : null;
  ...
  const similarProperties = useMemo(() => {
    ...
    return properties
      ...
      .map(p => ({
        ...p,
        distance: getDistanceKm(property.location[0], property.location[1], p.location[0], p.location[1]),
      }))
      .sort((a, b) => a.distance - b.distance)
      .slice(0, 4);
  }, [property, properties]);
  ...
  const galleryImages = useMemo(() => {
    ...
    if (property.pinInfo?.photoAtPin && property.pinInfo.photoAtPin !== property.image) {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-4">
            ...
            <h2 className="text-2xl font-bold text-gray-900">Không tìm thấy phòng trọ</h2>
            <p className="text-gray-600">Phòng trọ này không tồn tại hoặc đã bị xóa</p>
      ...
      <div className="bg-white border-b">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span>/</span>
            <button onClick={() => navigate('/map')} className="hover:text-green-600 transition-colors">Tìm trọ</button>
            <span>/</span>
        ...
        <div className="max-w-6xl mx-auto px-4 py-6">
            ...
            <div className="grid grid-cols-4 grid-rows-2 gap-2 h-[420px] rounded-2xl overflow-hidden">
              ...
              >
                ...
                <div className="absolute bottom-3 left-3 flex gap-2">
                  {property.pinInfo && (
                    <Badge className="bg-orange-500 text-white shadow-lg">📌 Chủ trọ đã ghim</Badge>
                  )}
                  <Badge variant={property.available ? 'default' : 'secondary'} className="shadow-lg">
              ...
              {galleryImages.slice(1, 5).map((img, i) => (
                ...
                >
                  <ImageWithFallback src={img} alt={`Ảnh ${i + 2}`} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-all" />
                  {/* Show more overlay on last */}
                  {i === 3 && galleryImages.length > 5 && (
                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                      <span className="text-white font-semibold text-lg">+{galleryImages.length - 5} ảnh</span>
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div>
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      />
                      {property.pinInfo && (
                        <Badge className="bg-orange-100 text-orange-800 border border-orange-200" variant="outline">
                          📌 GPS đã ghim
                        </Badge>
                      )}
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <Button
                      ...
                      className="h-10 w-10"
                    >
                      <Heart className={`size-5 ${favorite ? 'fill-red-500 text-red-500' : 'text-gray-500'}`} />
                    </Button>
                    <Button variant="outline" size="icon" onClick={handleShare} className="h-10 w-10">
                      {copied ? <Check className="size-5 text-green-600" /> : <Share2 className="size-5 text-gray-500" />}
                    </Button>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-6 bg-gradient-to-r from-blue-50 to-green-50 rounded-xl p-5 border border-blue-100">
                  <div>
                    <p className="text-3xl font-bold text-blue-600">{property.price.toLocaleString('vi-VN')}đ</p>
                  ...
                  <Separator orientation="vertical" className="h-12" />
                  <div>
                    <p className="text-2xl font-bold text-gray-900">{Math.round(property.price / property.area).toLocaleString('vi-VN')}đ</p>
              ...
              <div className="border-b">
                <div className="flex gap-1">
                  {[
                    ...
                    { key: 'reviews' as const, label: `Đánh giá (${reviews.length})`, icon: Star },
                    { key: 'nearby' as const, label: 'Tiện ích lân cận', icon: MapPin },
                  ].map(tab => (
                    <button
                      key={tab.key}
              ...
              {activeTab === 'info' && (
                <div className="space-y-6">
                  ...
                  {property.verificationLevel === 'location-verified' && property.locationAccuracy && (
                    <LocationVerificationInfo locationAccuracy={property.locationAccuracy} />
                  )}
                  ...
                  {property.pinInfo && (
                    <div className="bg-orange-50 border border-orange-200 rounded-xl p-5">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <div>
                          <h3 className="font-semibold text-orange-900">Chủ trọ đã ghim vị trí GPS</h3>
                          <p className="text-xs text-orange-600">
                            Ghim lúc {new Date(property.pinInfo.pinnedAt).toLocaleDateString('vi-VN')}
                          </p>
                        </div>
                      </div>
                  ...
                  {/* ⭐ GREEN BADGE - PLATFORM VERIFIED */}
                  {property.greenBadge && property.greenBadge.level === 'verified' && (
                    <div className="relative overflow-hidden rounded-2xl border-4 border-green-400 shadow-2xl" style={{ 
                      background: 'linear-gradient(135deg, #ecfdf5 0%, #d1fae5 50%, #a7f3d0 100%)',
                      boxShadow: '0 0 40px rgba(34, 197, 94, 0.4), 0 10px 30px rgba(0, 0, 0, 0.15)'
                    }}>
                      {/* Decorative corner */}
                      <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-green-400/30 to-transparent rounded-bl-full" />
                      <div className="absolute bottom-0 left-0 w-24 h-24 bg-gradient-to-tr from-green-400/20 to-transparent rounded-tr-full" />
                      ...
                      <div className="relative p-6">
                        ...
                        <div className="flex items-start gap-5 mb-6">
                          <div className="relative flex-shrink-0">
                            {/* Animated glow */}
                            <div 
                              className="absolute inset-0 bg-green-400 rounded-2xl blur-xl opacity-50 animate-pulse"
                              style={{ animation: 'pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite' }}
                            />
                            <div className="relative w-20 h-20 bg-gradient-to-br from-green-500 to-green-700 rounded-2xl flex items-center justify-center shadow-2xl transform hover:scale-110 transition-transform">
                              <ShieldCheck className="size-12 text-white" strokeWidth={2.5} />
                            </div>
                          </div>
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              <h3 className="text-2xl font-extrabold text-green-900">
                                ĐÃ ĐƯỢC XÁC THỰC BỞI NỀN TẢNG
                              </h3>
                              <div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center animate-bounce">
                                <Check className="size-5 text-white font-bold" strokeWidth={4} />
                              </div>
                            </div>
                            <p className="text-green-800 font-semibold text-base mb-2">
                              MapHome đã kiểm tra thực địa và xác nhận căn trọ này
                            </p>
                            <div className="flex items-center gap-2 text-sm text-green-700">
                              <Clock className="size-4" />
                              <span>
                                Xác thực ngày {new Date(property.greenBadge.awardedAt).toLocaleDateString('vi-VN')}
                              </span>
                            </div>
                            {property.greenBadge.inspectionNotes && (
                              <div className="mt-3 bg-white/70 backdrop-blur-sm rounded-xl p-3 border border-green-200">
                                <p className="text-sm text-green-900 italic">
                                  <span className="font-semibold">📝 Ghi chú kiểm tra:</span> "{property.greenBadge.inspectionNotes}"
                                </p>
                              </div>
                            )}
                          </div>
                        ...
                        <div className="bg-white/80 backdrop-blur-sm rounded-xl p-5 border-2 border-green-300 mb-5">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div className="flex gap-3 items-start p-3 bg-green-50 rounded-lg border border-green-200">
                              ...
                              <div>
                                <p className="font-semibold text-green-900 text-sm mb-1">100% Tin cậy</p>
                                <p className="text-xs text-green-800">Đã kiểm tra thực địa bởi đội ngũ MapHome, đảm bảo không phải tin ảo</p>
                              </div>
                            </div>
                            ...
                            <div className="flex gap-3 items-start p-3 bg-green-50 rounded-lg border border-green-200">
                              ...
                              <div>
                                <p className="font-semibold text-green-900 text-sm mb-1">Vị trí chính xác</p>
                                <p className="text-xs text-green-800">GPS đã được xác minh, bạn không lo bị dẫn nhầm địa chỉ</p>
                              </div>
                            </div>
                            ...
                            <div className="flex gap-3 items-start p-3 bg-green-50 rounded-lg border border-green-200">
                              ...
                              <div>
                                <p className="font-semibold text-green-900 text-sm mb-1">Thông tin đúng</p>
                                <p className="text-xs text-green-800">Giá, diện tích, tiện ích đã được kiểm chứng khớp với thực tế</p>
                              </div>
                            </div>
                            
                            <div className="flex gap-3 items-start p-3 bg-green-50 rounded-lg border border-green-200">
                              <div className="w-8 h-8 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5">
                                <User className="size-4 text-white" strokeWidth={2.5} />
                              </div>
                              <div>
                                <p className="font-semibold text-green-900 text-sm mb-1">Chủ trọ uy tín</p>
                                <p className="text-xs text-green-800">Đã hợp tác với MapHome, cam kết chất lượng dịch vụ</p>
                              </div>
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                              <h4 className="font-bold text-blue-900 mb-2 text-base">
                                💎 Dịch vụ xác thực cao cấp
                              </h4>
                              <p className="text-sm text-blue-800 mb-3">
                                Chủ trọ đã đầu tư <span className="font-bold text-blue-600">299.000đ - 499.000đ</span> để được đội ngũ MapHome kiểm tra thực địa, chụp ảnh, xác minh thông tin. Điều này chứng tỏ sự nghiêm túc và cam kết chất lượng!
                              </p>
                        ...
                        {/* Trust badge */}
                        <div className="mt-5 flex items-center justify-center gap-3 py-3 bg-gradient-to-r from-green-600 to-blue-600 rounded-xl shadow-lg">
                          <Shield className="size-5 text-white" />
                          <p className="text-white font-bold text-sm">
                            TRUST IS KING - Bảo vệ người thuê trọ khỏi tin ảo
                          </p>
                          <Shield className="size-5 text-white" />
                        </div>
                      </div>
                  ...
                  {/* Show CTA to get verified if not verified */}
                  {(!property.greenBadge || property.greenBadge.level === 'none') && (
                    <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-5 border-2 border-amber-300 shadow-lg">
                      <div className="flex items-start gap-4 mb-4">
                        <div className="w-12 h-12 rounded-xl bg-amber-500 flex items-center justify-center flex-shrink-0 shadow-md">
                          <ShieldCheck className="size-6 text-white" />
                        </div>
                        <div className="flex-1">
                          <h4 className="font-bold text-amber-900 mb-2 text-lg">Căn trọ chưa được xác thực</h4>
                          <p className="text-sm text-amber-800 mb-3">
                            Tin đăng này chưa được MapHome kiểm tra thực địa. Bạn có thể yêu cầu admin đến kiểm tra để bảo vệ bản thân khỏi tin ảo!
                          </p>
                          <div className="flex flex-wrap gap-2 text-xs text-amber-700 mb-4">
                            <span>⚠️ Kiểm tra giấy tờ chủ nhà</span>
                            <span>•</span>
                            <span>⚠️ Xem phòng trực tiếp</span>
                            <span>•</span>
                            <span>⚠️ Không chuyển tiền trước</span>
                          </div>
                          
                          {/* CTA Button */}
                          <Button
                            onClick={() => setIsRequestInspectionOpen(true)}
                          ...
                          >
                            ...
                            Yêu cầu Admin kiểm tra miễn phí
                          </Button>
                          <p className="text-xs text-amber-600 mt-2 text-center">
                            💡 Miễn phí cho người thuê trọ - Admin sẽ đến thực địa xác minh
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Amenities */}
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 mb-4">Tiện ích phòng</h3>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {activeAmenities.map(key => {
                        ...
                        const Icon = cfg.icon;
                        return (
                          <div key={key} className="flex items-center gap-3 p-3 bg-green-50 rounded-xl border border-green-100">
                            <div className="w-10 h-10 rounded-lg bg-green-100 flex items-center justify-center">
                              <Icon className="size-5 text-green-700" />
                            </div>
                            <span className="text-sm font-medium text-green-900">{cfg.label}</span>
                          </div>
                        );
                      })}
                      {inactiveAmenities.map(key => {
                        ...
                        const Icon = cfg.icon;
                        return (
                          <div key={key} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100 opacity-50">
                            <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center">
                              <Icon className="size-5 text-gray-400" />
                            </div>
                            <span className="text-sm text-gray-400 line-through">{cfg.label}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  {/* Mini Map */}
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 mb-4">Vị trí trên bản đồ</h3>
              ...
              <div className="lg:sticky lg:top-24 space-y-5">
                ...
                <Card className="shadow-lg border-green-200 overflow-hidden">
                  <div className="bg-gradient-to-r from-green-600 to-blue-600 p-4 text-white">
                    <p className="text-2xl font-bold">{property.price.toLocaleString('vi-VN')}đ<span className="text-base font-normal opacity-80">/tháng</span></p>
                    <p className="text-sm opacity-80 mt-1">{property.area}m² • {property.address.split(',').pop()?.trim()}</p>
                  </div>
                  <CardContent className="p-4 space-y-3">
                    ...
                    <Button
                      ...
                      className="w-full"
                      size="lg"
                      onClick={() => window.location.href = `tel:${property.phone.replace(/\s/g, '')}`}
                    >
                      <Phone className="size-5 mr-2" />
                      Gọi {property.phone}
                    </Button>
                    <Button variant="outline" className="w-full text-green-700 border-green-300 hover:bg-green-50" size="lg">
                    ...
                    <CardContent className="p-5">
                      ...
                      <div className="grid grid-cols-3 gap-3 mb-4">
                        <div className="text-center p-2 bg-gray-50 rounded-lg">
                          <p className="text-lg font-bold text-gray-900">{landlord.totalListings}</p>
                          <p className="text-[10px] text-gray-500">Tin đăng</p>
                        </div>
                        <div className="text-center p-2 bg-gray-50 rounded-lg">
                          <p className="text-lg font-bold text-green-600">{landlord.responseRate}%</p>
                          <p className="text-[10px] text-gray-500">Phản hồi</p>
                        </div>
                        <div className="text-center p-2 bg-gray-50 rounded-lg">
                          <p className="text-sm font-bold text-blue-600">{landlord.responseTime}</p>
                      ...
                      <div className="text-xs text-gray-500 flex items-center gap-1">
                        ...
                        Tham gia từ {new Date(landlord.joinedDate).toLocaleDateString('vi-VN', { month: 'long', year: 'numeric' })}
                      </div>
                    </CardContent>
                  </Card>
                )}
          ...
          <div className="mt-12">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-900">Phòng trọ tương tự gần đây</h2>
              ...
              {similarProperties.map(sp => (
                ...
                >
                  <div className="relative h-40 overflow-hidden">
                    ...
                    />
                    {sp.pinInfo && (
                      <Badge className="absolute top-2 left-2 bg-orange-500 text-white text-[10px] shadow">📌 Đã ghim</Badge>
                    )}
                    <Badge
                  ...
                  <CardContent className="p-3">
                    <h3 className="font-semibold text-sm text-gray-900 truncate mb-1">{sp.name}</h3>
                    <p className="text-xs text-gray-500 truncate mb-2">{sp.address}</p>
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-bold text-blue-600">{sp.price.toLocaleString('vi-VN')}đ</p>
                      <span className="text-xs text-gray-400">{sp.area}m² • {sp.distance < 1 ? `${Math.round(sp.distance * 1000)}m` : `${sp.distance.toFixed(1)}km`}</span>
                    </div>
                  </CardContent>
                </Card>
              ))}
