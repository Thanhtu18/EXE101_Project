...
export function LandlordDashboardV2() {
  ...
  const landlordPosts = properties.filter(p => 
    p.ownerName === 'Chủ trọ mới' || p.pinInfo
  );

  // Get verification requests
  const verificationRequests = getRequestsByLandlord('landlord-demo');
      ...
      case 'verification':
        return (
          <div className="space-y-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Yêu cầu xác thực</h2>
              <p className="text-gray-600">Quản lý các yêu cầu kiểm tra và cấp tích xanh</p>
            </div>
                  ...
                  {verificationRequests.map((req) => (
                    <div key={req.id} className="p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
                      <div className="flex items-center justify-between mb-2">
                        <p className="font-semibold text-gray-900">{req.propertyName}</p>
                        <span
                      ...
                      <p className="text-sm text-gray-600 mb-2">📍 {req.propertyAddress}</p>
                      <p className="text-sm text-gray-600">
                        📅 {new Date(req.scheduledDate).toLocaleDateString('vi-VN')} • {req.scheduledTime}
                      </p>
                      {req.note && (
                        <p className="text-xs text-gray-500 mt-2 italic">Ghi chú: {req.note}</p>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-6">
            ...
            {activeTab === 'overview' && (
              <>
                <div>
                  ...
                  <p className="text-gray-600">Theo dõi hoạt động tin đăng của bạn</p>
                </div>
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-900">
                {activeTab === 'posts' ? 'Tin đăng của bạn' : 'Tin đăng gần đây'}
              </h3>
              <Button
            ...
            <div className="space-y-4">
              {landlordPosts.map((post) => {
                const verificationLevel = post.verificationLevel === 'location-verified' ? 3 
                  : post.verificationLevel === 'phone-verified' ? 2 
                  : post.verificationLevel ? 1 : 0;
