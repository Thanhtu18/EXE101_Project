...
export function SettingsView() {
  ...
  const handleSavePlan = async () => {
    try {
      ...
      setSaving(true);
      if (isCreating) {
        const res = await api.post("/api/admin/subscriptions/plans", editingPlan);
        if (res.status === 201) toast.success("Thêm gói mới thành công! ✨");
      } else {
        const res = await api.put(`/api/admin/subscriptions/plans/${editingPlan._id || editingPlan.planId}`, editingPlan);
  ...
  const handleDeletePlan = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!window.confirm("Bác có chắc muốn xóa vĩnh viễn gói dịch vụ này?")) return;
    try {
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-indigo-500/70 mt-1 font-medium">Điều chỉnh các tham số vận hành cốt lõi của nền tảng</p>
        </div>
        <div className="flex gap-3">
          ...
          >
            <Save className="size-4 mr-2" /> {saving ? "Đang lưu..." : "Lưu thay đổi"}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-[260px_1fr] gap-8 items-start">
        ...
        <div className="bg-white rounded-[32px] border border-slate-100 p-3 shadow-sm space-y-2 sticky top-24">
          <TabNav active={activeTab === "general"} onClick={() => setActiveTab("general")} icon={<Globe />} label="Chung" />
          <TabNav active={activeTab === "pricing"} onClick={() => setActiveTab("pricing")} icon={<DollarSign />} label="Dịch vụ & Giá" />
          <TabNav active={activeTab === "broadcast"} onClick={() => setActiveTab("broadcast")} icon={<Bell />} label="Truyền thông" />
          <TabNav active={activeTab === "subscription_plans"} onClick={() => setActiveTab("subscription_plans")} icon={<Layers />} label="Gói dịch vụ" />
          <div className="pt-2 mt-2 border-t border-slate-50">
            <TabNav active={activeTab === "account"} onClick={() => setActiveTab("account")} icon={<User />} label="Tài khoản" />
          </div>
        </div>
            ...
            >
              {activeTab === "general" && (
                <div className="space-y-8">
                  <SectionHeader title="Thông tin nền tảng" description="Cập nhật các thông tin hiển thị cơ bản của website." />
                    ...
                    <div className="flex items-center justify-between p-6 bg-rose-50/30 rounded-[32px] border border-rose-100/50 group">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-rose-600 font-medium">Tạm khóa toàn bộ truy cập từ phía người dùng.</p>
                        </div>
                      </div>
              ...
              {activeTab === "pricing" && (
                <div className="space-y-8">
                  <SectionHeader title="Cấu hình Giá Dịch vụ" description="Thiết lập chi phí cho các gói xác thực Tích Xanh." />
                  <div className="grid grid-cols-2 gap-8">
                    <div className="p-8 bg-blue-50/30 rounded-[32px] border border-blue-50 space-y-6">
                      <div className="flex items-center gap-3 text-blue-700">
                        ...
                        <span className="text-xs font-black uppercase tracking-widest">Xác thực Cơ bản</span>
                      </div>
                      <InputGroup label="Chi phí (VNĐ)" type="number" value={settings.pricing.basicVerification} onChange={(val) => setSettings({...settings, pricing: {...settings.pricing, basicVerification: parseInt(val)}})} />
                      <p className="text-[10px] text-blue-400 font-bold uppercase italic tracking-tighter">Áp dụng xác minh qua SĐT/Zalo</p>
                    </div>
                    ...
                    <div className="p-8 bg-amber-50/30 rounded-[32px] border border-amber-50 space-y-6">
                      <div className="flex items-center gap-3 text-amber-700">
                        ...
                        <span className="text-xs font-black uppercase tracking-widest">Xác thực Thực địa</span>
                      </div>
                      <InputGroup label="Chi phí (VNĐ)" type="number" value={settings.pricing.premiumVerification} onChange={(val) => setSettings({...settings, pricing: {...settings.pricing, premiumVerification: parseInt(val)}})} />
                      <p className="text-[10px] text-amber-400 font-bold uppercase italic tracking-tighter">Xác minh tận nơi bởi MapHome</p>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between p-6 bg-emerald-50/30 rounded-[32px] border border-emerald-100/50">
                      ...
                      <label className="relative inline-flex items-center cursor-pointer scale-110">
                        <input type="checkbox" className="sr-only peer" checked={settings.isBroadcastEnabled} onChange={(e) => setSettings({...settings, isBroadcastEnabled: e.target.checked})} />
              ...
              {activeTab === "subscription_plans" && (
                <div className="space-y-8">
                    <div className="flex items-center justify-between">
                        <SectionHeader title="Gói Đăng ký Dịch vụ" description="Quản lý đặc quyền và mức phí dành cho Chủ trọ." />
                        <Button 
                            onClick={() => {
                                setIsCreating(true);
                                setEditingPlan({ planId: "", name: "", price: 0, yearlyPrice: 0, description: "", features: [{ text: "", included: true }], badge: "", badgeColor: "bg-gray-100", icon: "Star" });
                            }}
                        ...
                        >
                          <div className="flex items-center gap-6">
                            <div className="w-16 h-16 bg-slate-50 rounded-[22px] flex flex-col items-center justify-center text-emerald-600 border border-slate-100 shadow-inner group-hover:bg-emerald-500 group-hover:text-white transition-all">
                              <span className="text-sm font-black leading-none">{plan.price >= 1000 ? (plan.price / 1000) + 'k' : plan.price}</span>
                              <span className="text-[10px] uppercase font-black tracking-tighter mt-0.5 opacity-60">VNĐ</span>
                            </div>
                            <div>
                               <div className="flex items-center gap-2 mb-1">
                                  <h5 className="font-black bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent tracking-tight">{plan.name}</h5>
                                  {plan.badge && <span className={`text-[9px] px-2.5 py-1 rounded-lg font-black uppercase tracking-widest ${plan.badgeColor || 'bg-slate-100 text-slate-500'}`}>{plan.badge}</span>}
                               </div>
                               <p className="text-xs text-slate-400 font-bold max-w-sm truncate">{plan.description}</p>
                            </div>
                          </div>
                          <div className="flex gap-2">
                             <Button variant="outline" size="sm" onClick={() => setEditingPlan(plan)} className="h-10 rounded-xl border-slate-100 text-slate-600 font-black text-[10px] uppercase tracking-widest px-4 hover:bg-slate-50">
                                Sửa
                             </Button>
                             <Button variant="ghost" size="sm" onClick={(e) => handleDeletePlan(plan._id || plan.planId, e)} className="h-10 rounded-xl text-slate-300 font-black text-[10px] uppercase tracking-widest px-4 hover:text-rose-500 hover:bg-rose-50">
                    ...
                    <AnimatePresence>
                      {editingPlan && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                           <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setEditingPlan(null)} />
                           <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white rounded-[40px] shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar p-10 border border-slate-100">
                              <h3 className="text-2xl font-black bg-gradient-to-r from-emerald-600 to-blue-600 bg-clip-text text-transparent mb-8">{isCreating ? "Thêm Gói Đặc Quyền Mới" : "Chỉnh sửa Gói"}</h3>
                              
                              <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-6">
                                  <InputGroup label="Tên Gói (VD: Pro, V.I.P)" value={editingPlan.name} onChange={(val) => setEditingPlan({...editingPlan, name: val})} />
                                  <InputGroup label="Mô tả ngắn gọn" value={editingPlan.description} onChange={(val) => setEditingPlan({...editingPlan, description: val})} />
                                  <InputGroup label="Icon (Lucide name)" value={editingPlan.icon || "Star"} onChange={(val) => setEditingPlan({...editingPlan, icon: val})} />
                                  <InputGroup label="Huy hiệu (Badge Text)" value={editingPlan.badge || ""} onChange={(val) => setEditingPlan({...editingPlan, badge: val})} />
                                  <InputGroup label="Giá Mỗi Tháng (VNĐ)" type="number" value={editingPlan.price} onChange={(val) => setEditingPlan({...editingPlan, price: Number(val)})} />
                                  <InputGroup label="Giá Mỗi Năm (VNĐ)" type="number" value={editingPlan.yearlyPrice} onChange={(val) => setEditingPlan({...editingPlan, yearlyPrice: Number(val)})} />
                                </div>

                                <div className="pt-6 border-t border-slate-100 space-y-4">
                                  <div className="flex items-center justify-between">
                                     <label className="text-[11px] font-black text-indigo-500/60 uppercase tracking-widest">Danh sách Quyền lợi</label>
                                     <Button size="sm" variant="outline" onClick={() => setEditingPlan({...editingPlan, features: [...(editingPlan.features || []), { text: "", included: true }]})} className="h-8 rounded-lg text-xs font-black"><Plus className="size-3 mr-1"/> Thêm quyền lợi</Button>
                                  </div>
                                  {(editingPlan.features || []).map((feature: any, idx: number) => (
                                     <div key={idx} className="flex items-center gap-3">
                                        <button onClick={() => {
                                          const newFeatures = [...editingPlan.features];
                                          newFeatures[idx].included = !newFeatures[idx].included;
                                        ...
                                        <input value={feature.text} onChange={(e) => {
                                          ...
                                          setEditingPlan({...editingPlan, features: newFeatures});
                                        }} className="flex-1 h-10 px-4 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-emerald-500" placeholder="VD: Đăng ký tối đa 5 phòng" />
                                        <button onClick={() => {
                              ...
                              <div className="flex items-center justify-end gap-3 mt-10">
                                 <Button variant="outline" onClick={() => setEditingPlan(null)} className="h-12 rounded-2xl px-6 font-black text-xs uppercase tracking-widest text-slate-500">Hủy</Button>
                                 <Button onClick={handleSavePlan} disabled={saving} className="h-12 rounded-2xl px-8 font-black text-xs uppercase tracking-widest bg-emerald-600 hover:bg-emerald-700 text-white shadow-xl shadow-emerald-100">{saving ? "Đang lưu..." : "Lưu Gói Dịch Vụ"}</Button>
                              </div>
                           </motion.div>
                        </div>
                      )}
              ...
              {activeTab === "account" && (
                <div className="space-y-10">
                  <SectionHeader title="Quản trị viên" description="Thông tin cá nhân và bảo mật tài khoản." />
                  <div className="flex flex-col items-center gap-6 pb-10 border-b border-slate-50">
                     <div className="relative group">
                        <div className="w-32 h-32 rounded-[40px] border-[6px] border-white shadow-2xl overflow-hidden bg-gradient-to-br from-emerald-500 to-indigo-600 flex items-center justify-center text-white text-4xl font-black group-hover:scale-105 transition-transform duration-500">
                           {user?.avatar ? <img src={getAvatarUrl(user.avatar) || ""} className="w-full h-full object-cover" /> : getInitials(user?.fullName, user?.username)}
                        </div>
...
function TabNav({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) {
  return (
    ...
    >
      ...
      {active && <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute right-4 w-1.5 h-1.5 bg-white rounded-full relative z-10" />}
    </button>
  );
}
...
function SectionHeader({ title, description }: { title: string; description: string }) {
  return (
    <div>
      <h3 className="text-base font-black bg-gradient-to-r from-emerald-600 to-blue-600 bg-clip-text text-transparent uppercase tracking-widest leading-none mb-1">{title}</h3>
      <p className="text-xs font-bold text-indigo-500/50 italic mt-1.5">{description}</p>
    </div>
  );
}
...
function InputGroup({ label, value, onChange, icon, type = "text" }: { label: string; value: any; onChange: (val: string) => void; icon?: React.ReactNode, type?: string }) {
  return (
    <div className="space-y-2">
      <label className="block text-[11px] font-black text-indigo-500/60 uppercase tracking-widest ml-1">{label}</label>
      <div className="relative group">
